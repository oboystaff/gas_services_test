$(document).ready(function () {
    ...
    $('.generate_report').on('click', function(event) {
        ...
        if ($('select[name="report_type"]').val() == 1) {
            ...
            load_data(from_date, to_date, report_type);
        } else {
            $('#summary').show();
            $('#details').hide();
            $('#graph').show();
    ...
    function load_data(from_date = '', to_date = '', report_type = '') {
        var url = $("input[name='agent-report_url']").attr("url");
        ...
        try {
            $('#zero_config').DataTable({
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    // Update status DIV
                    $('#title').html('Total');
                    $('#delivery_total').html(delivery_total);
                }
            });
        } catch (err) {
