$(document).ready(function () {
    ...
    $('.generate_report').on('click', function(event) {
        ...
        if ($('select[name="report_type"]').val() == 1) {
            ...
            load_data(from_date, to_date, branch_id, report_type);
        } else {
            $('#summary').show();
            $('#details').hide();
            $('#graph').show();
    ...
    function load_data(from_date = '', to_date = '', branch_id = '', report_type = '') {
        var url = $("input[name='receivables-report_url']").attr("url");
        ...
        try {
            $('#zero_config').DataTable({
                ...
                columns: [
                    ...
                    {data: 'name', name: 'name'},
                    {data: 'total_invoiced', name: 'total_invoiced'},
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    // Update status DIV
                    $('#title').html('Total');
                    $('#total_invoice').html(format_money(total_invoice));
                    $('#total_paid').html(format_money(total_paid));
                    $('#receivable').html(format_money(receivable));
                }
            });
        } catch (err) {
