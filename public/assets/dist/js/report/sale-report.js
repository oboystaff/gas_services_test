$(document).ready(function () {
    ...
    $('.generate_report').on('click', function(event) {
        ...
        var community_id = $('select[name="community_id"]').val();
        ...
        if ($('select[name="report_type"]').val() == 1) {
            ...
            load_data(from_date, to_date, branch_id, community_id, user_id, report_type);
        } else {
            $('#summary').show();
            $('#details').hide();
            //$('#graph').show();
            $('#zero_config2').DataTable().destroy();
            load_summary(from_date, to_date, user_id);
        }
    });
    ...
    function load_data(from_date = '', to_date = '', branch_id = '', community_id = '', user_id = '', report_type = '') {
        var url = $("input[name='sale-report_url']").attr("url");
        ...
        try {
            $('#zero_config').DataTable({
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    $('#service_charge_total').html(format_money(service_charge_total));
                }
            });
        } catch (err) {
    ...
    function load_summary(from_date = '', to_date = '', user_id = '') {
        var url = $("input[name='sale-report_url']").attr("url");
            ...
            $('#zero_config2').DataTable({
               ...
               columns: [
                ...
                "footerCallback": function () {
                    ...
                    // Update status DIV
                    $('#title2').html('Total');
                    $('#customer_total').html(format_money(customer_total));
                }
            });
        } catch (err) {
            //alert(err);
